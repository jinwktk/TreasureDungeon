...
- **v2.6.0 yield vnav移行問題緊急修正**: ユーザー報告「動かなくなった。元に戻してほしい」への即座対応
- **IPC API方式復元**: USE_YIELD_VNAV=trueからDISABLE_IPC_VNAV=falseに設定変更
- **VNavMoveTo/Flag/Target関数修正**: yield vnav系コマンドからIPC.vnavmesh.PathfindAndMoveToに復元
- **移動システム安定化**: 動作実績のあるIPC APIベースシステムに戻し正常動作確保
- **フォールバック機能維持**: IPC API失敗時の従来yieldコマンド実行を保持
- **v2.6.0完全取消**: yield vnav系コマンド完全移行を取り消しシステム安定性優先
