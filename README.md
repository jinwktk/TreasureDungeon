...
**設計**: 新SND v12.0.0+の最新API構造に完全適合

### v4.0.0での主な改善点
- **API完全対応**: 新SND v12.0.0+のモジュールベースAPIに完全対応
- **ゾーン検出最適化**: GetZoneID()関数対応・リムサ検出の信頼性向上
- **プレイヤー状態管理**: Player.*プロパティ対応・フォールバック機能
- **距離計算改善**: GetDistanceToTarget()新API対応・手動計算併用
- **互換性強化**: 旧API関数との併用による動作安定性向上
