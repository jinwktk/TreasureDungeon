...
## 最新バージョン情報 (v1.0.0) - 安定版リリース
**作成日**: 2025-07-13  
**API対応**: SomethingNeedDoing v12.0.0+完全対応  
**設計**: SEHException完全対策・安定性最優先設計

### v1.0.0 安定版での主要改善点
- **SEHException完全対策**: IPC.vnavmesh.PathfindAndMoveTo無効化・NLua低レベルエラー回避
- **無限ループ修正**: ドマ反乱軍の門兵インタラクト後の適切なターゲット解除・フラグ管理
- **包括的エラー処理**: 多層SafeExecute・Lua状態健全性チェック・システムレベル例外検出
- **安定性優先設計**: yield専用移動・危険API完全回避・時間計算オーバーフロー防止
- **Y座標補正システム**: 既知問題座標での自動Y値修正・フラグ移動精度向上
