...
## 最新バージョン情報 (v1.7.0) - 食事バフ自動再摂取機能実装
**作成日**: 2025-07-17  
**API対応**: SomethingNeedDoing v12.0.0+完全対応  
**新機能**: 食事バフ残り時間10分以下で自動再摂取

### v1.7.0 食事バフ自動再摂取機能の特徴
- **正確な残り時間取得**: `GetStatusTimeRemaining(48)`で食事バフ（Well Fed）の残り時間を秒単位で取得
- **10分閾値制御**: 残り時間が10分（600秒）以下になると自動でCtrl+Shift+F9を実行
- **戦闘中安全制御**: 戦闘中は食事実行をスキップして安全性を確保
- **詳細ログ機能**: 残り時間を分秒表示で正確に表示し、実行タイミングを明確化
- **30秒間隔チェック**: 定期的な残り時間監視で効率的な食事バフ管理
